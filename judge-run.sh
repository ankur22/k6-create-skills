...
#   ./judge-run.sh <scripts-dir> --no-blind                Keep `// Generated by ...` headers visible to the judge
...
#
# By default, skill-identifying headers like `// Generated by k6-create-xk6docs on ...`
# are stripped from the script before it is shown to the judge LLM. Pass --no-blind
# to disable this and judge the script exactly as it sits on disk.
#
...
BLIND_LABEL="$([[ -n "$NO_BLIND" ]] && echo "no (--no-blind)" || echo "yes (skill-identity stripped)")"

echo "Judge model: $JUDGE_LABEL" >&2
echo "Gold dir:    $GOLD_LABEL" >&2
echo "Scripts dir: $SCRIPTS_DIR" >&2
echo "Blinding:    $BLIND_LABEL" >&2
...
judge_one() {
  ...
  [[ -n "$NO_BLIND" ]] && judge_args+=(--no-blind)

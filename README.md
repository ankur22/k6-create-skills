...
Scripts generated by `claude-opus-4-6` and scored by a blinded `claude-opus-4-6` judge
with deterministic hard caps from `bp-check.py`.

| Condition | Valid | BP score | Tokens (avg) | LLM Judge (avg /25) |
|-----------|:-----:|:--------:|:------------:|:-------------------:|
| **k6-create-xk6docs** | 29/29 | 99% | ~35k | **24.3** |
| **grafana-k6** | 29/29 | 97% | ~36k | 23.8 |
| **mcp-k6** (raw, no skill) | 29/29 | 95% | ~48k | 23.1 |

`k6-create-xk6docs` leads on both quality and token efficiency. The raw MCP server
costs ~37% more tokens than the skill-based approach for slightly weaker scores.
Generating with the cheaper `claude-sonnet-4-6` gives nearly identical scores
(23.5–23.9/25); opus generation only meaningfully helps `k6-create-xk6docs` on one
scenario it previously failed to produce.
...
All 29 scenarios are also checked against `scenario-manifest.json` for exact
requirements (endpoints, methods, executor, VUs, durations, thresholds, screenshots,
etc.). Style-preference checks (e.g. preferring `expect()` over `check()`) live in a
separate `style` category that does not gate the judge.
...
By default the judge is **blinded**: skill-identifying `// Generated by ...` headers
are stripped from each script before it is sent to the judge LLM, so the LLM cannot
fingerprint which skill produced it. Pass `--no-blind` to `judge-run.sh` to disable
this (useful for debugging or replicating earlier non-blinded runs).
